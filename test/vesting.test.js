...
describe('Vesting', async () => {
  ...
  const scoringEpochSize = 50;
    ...
    describe('#updateScore', async () => {
      ...
      it('score value range', async () => {
        ...
        expectEvent(receipt, 'ScoreUpdated', { score: "100", epochNumber: "1" });
  ...
  describe('vesting simulation', async () => {
    ...
    it('demo', async () => {
      ...
      const scores = [0, 10, 12, 0, 56, 20, 90];
      ...
      for (let i = 0; i < scores.length; i++) {
        ...
        if (100 < accumulatedScore) accumulatedScore = 100;
        for (let i = 0; i < numberOfBeneficiaries; i++) {
      ...
      vDetails = await this.vesting.locks(accounts[0]);
      assert.equal(
        vDetails.releasedAmount.toString(),
        vDetails.totalAmount.toString()
      );
